...
exports.addStore = async (req, res) => {
    try {
        ...
        if (!s_Name || !latitude || !longitude) {return res.status(400).send({message: "ข้อมูลไม่ครบถ้วน (ชื่อร้าน, พิกัด)",status: 0});
        }
        ...
        io.emit('refreshStores');
        return res.status(201).send({message: "บันทึกข้อมูลร้านค้าสำเร็จ",status: 1,data: { s_ID: result.insertId,...req.body}});
    } catch (error) {
        console.error("Error adding store:", error);
        return res.status(500).send({ message: "เกิดข้อผิดพลาดในการบันทึกข้อมูล",status: 0,error: error.message});
    }
}
...
exports.updateStore = async (req, res) => {
    ...
    try {
        ...
        if (result.affectedRows > 0) {
            io.emit('refreshStores');
            return res.status(200).json({  status: 1,  message: "อัปเดตข้อมูลร้านค้าสำเร็จ" });
        } else {
            return res.status(404).json({ status: 0, message: "ไม่พบข้อมูลร้านค้าที่ต้องการอัปเดต" });
        }

    } catch (error) {
        console.error("Update Store Error:", error);
       return res.status(500).json({    status: 0,   message: "เกิดข้อผิดพลาดที่เซิร์ฟเวอร์"   });
    }
};
exports.deleteStore = async (req, res) => {
    ...
    try {
        ...
        if (result.affectedRows > 0) {
            io.emit('refreshStores');
            return res.status(200).json({status: 1,message: "ลบข้อมูลร้านค้าเรียบร้อยแล้ว" });
        } else {
            return res.status(400).json({ status: 0,message: "ลบข้อมูลไม่สำเร็จ"});
        }
    } catch (error) {
