...
exports.getallOrder = async (req, res)=>{
    try {
        ...
        if(orderResult.length === 0 ){
            return res.status(200).send({message : `No Orders Data`})
        }
        const data = orderResult
        return res.status(200).send({message : "Select Order Data Successfully" , data})
    } catch (error) {
...
exports.addOrder = async (req,res) => {
    try {
        ...
        if(orderResult.affectedRows === 0) {
            return res.stats(400).send({message : `Can't Insert Order `})
        }
        ...
        for ( const item of cart) {
            ...
            if (orderItemResult.affectedRows === 0) {
                return res.status(400).send({message : `Can't Insert Order items`})
            }
        }
        return res.status(201).send({message : `Order Success`})
    } catch (error) {
        console.log(error)
